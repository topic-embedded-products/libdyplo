...
{
	unsigned int* f = (unsigned int*)&cfg;
	os << "[" << f[0];
	for (unsigned int i = 1; i < sizeof(cfg)/sizeof(unsigned int); ++i)
		os << "," << f[i];
	os << "]";
	return os;
}
...
TEST(hardware_driver_ctx, q_dma_standalone_config_readback)
{
	...
	static const unsigned int blocksize = 64 * 1024;
	static const unsigned int num_blocks = 2;
	/* Standalone mode requires special flag */
	dyplo::HardwareDMAFifo dma_sa(context.openDMA(dma_index, O_RDWR|O_SYNC));
	dma_sa.reconfigure(dyplo::HardwareDMAFifo::MODE_STANDALONE, blocksize, num_blocks, false);
	EQUAL(num_blocks, dma_sa.count());

	dyplo::HardwareDMAFifo::StandaloneConfiguration cfg1;
	memset(&cfg1, 0, sizeof(cfg1));
	cfg1.offset = 0;
	cfg1.burst_size = 256;
	cfg1.incr_a = 256;
	cfg1.iterations_a = 256;
	dma_sa.setStandaloneConfig(&cfg1, false);

	dyplo::HardwareDMAFifo::StandaloneConfiguration cfg2;
	memset(&cfg2, 0xFF, sizeof(cfg2));
	dma_sa.getStandaloneConfig(&cfg2, false);

	/* EQUAL(cfg1, cfg2); */
	if (cfg1 != cfg2)
	{
		std::ostringstream msg;
		msg << "Configuration readback failed, expected: " << cfg1 << " != actual: " << cfg2;
		FAIL(msg.str());
	}
}
...
TEST(hardware_driver_ctx, q_dma_standalone_pass)
{
	...
	/* Standalone mode requires special flag */
	dyplo::HardwareDMAFifo dma_sa(context.openDMA(standalone_dma_index, O_RDWR|O_SYNC));
	dma_sa.reconfigure(dyplo::HardwareDMAFifo::MODE_STANDALONE, blocksize, num_blocks, false);
	EQUAL(num_blocks, dma_sa.count());

	/* Setup a transfer set that just copies the data without changes,
	 * since 64*1024 == 256*256 */
	dyplo::HardwareDMAFifo::StandaloneConfiguration cfg1;
	memset(&cfg1, 0, sizeof(cfg1));
	cfg1.offset = 0;
	cfg1.burst_size = 64;
	cfg1.incr_a = 64;
	cfg1.iterations_a = 1024;
	dma_sa.setStandaloneConfig(&cfg1, false);
	dyplo::HardwareDMAFifo::StandaloneConfiguration cfg2;
	...
	CHECK(!from_logic.poll_for_incoming_data(0));
}

static void standalone_transform(char* dst, const char* src,
	const dyplo::HardwareDMAFifo::StandaloneConfiguration cfg1)
{
	unsigned int a = 0;
	unsigned int b = 0;
	unsigned int c = 0;
	...
	src += cfg1.offset;
	for (;;) //unsigned int i = 0; i < blocksize; i += cfg1.burst_size)
	{
		memcpy(dst + offset, src, cfg1.burst_size);
		src += cfg1.burst_size;
		++a;
		if (a < cfg1.iterations_a)
		{
			offset += cfg1.incr_a;
		}
		else
		{
			a = 0;
			++b;
			if (b < cfg1.iterations_b)
			{
				offset = b * cfg1.incr_b;
			}
			else
			{
				b = 0;
				++c;
				if (c < cfg1.iterations_c)
				{
					offset = c * cfg1.incr_c;
				}
				else
					break; /* no more data */
			}
...
TEST(hardware_driver_ctx, q_dma_standalone_rearrange_2D)
{
	...
	/* Setup a transfer set that re-arranges data in a pattern */
	dyplo::HardwareDMAFifo::StandaloneConfiguration cfg1;
	memset(&cfg1, 0, sizeof(cfg1));
	cfg1.offset = 0;
	cfg1.burst_size = 64;
	...
	cfg1.incr_b = 64;
	cfg1.iterations_b = 4;
	dma_sa.setStandaloneConfig(&cfg1, true);
	dyplo::HardwareDMAFifo::StandaloneConfiguration cfg2;
	memset(&cfg2, 0, sizeof(cfg2));
	cfg2.offset = 0;
	cfg2.burst_size = 256;
	...
	std::vector<unsigned int> testdata(blocksize_words);
	std::vector<unsigned int> testresult(blocksize_words);
	std::vector<unsigned int> expected(blocksize_words);
	...
	standalone_transform((char*)&expected[0], (const char*)&testdata[0], cfg1);

	for (unsigned int i = 0; i < blocksize_words; ++i)
	{
		EQUAL(expected[i], testresult[i]);
	}
